...
describe('workflow save durable reopen', () => {
  it('persists a committed workflow and can reopen it after the database closes', async () => {
    ...
    try {
      ...
      store.saveWorkflow({ inputs: [], id: 'workflow-persisted', name: '영속성 테스트', goal: '커밋된 workflow를 다시 읽기', version: 1, trigger: { type: 'manual' }, steps: [], permissions: {}, approval: [], allowExternalAuto: true, assumptions: [], sideEffects: {}, dataPolicy: {} });
      db.close?.();
