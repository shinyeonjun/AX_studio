...
describe('workflow save version and activation', () => {
  it('allocates a new monotonic version for every save', async () => {
    ...
    const workflow = { inputs: [], id: 'workflow-1', name: '버전 테스트', goal: '저장 버전 확인', version: 1, steps: [], permissions: {}, approval: [], allowExternalAuto: true, assumptions: [], sideEffects: {}, dataPolicy: {} };
  ...
  it('persists new workflows as disabled until explicitly enabled', async () => {
    const db = await createDatabaseAsync(':memory:');
    const store = new WorkflowStore(db);
    const { workflowId } = store.saveWorkflow({ inputs: [], id: 'wf-disabled', name: '비활성 저장', goal: 'test', version: 1, steps: [], permissions: {}, approval: [], allowExternalAuto: true, assumptions: [], sideEffects: {}, dataPolicy: {} });
