...
export function countWorkspaceSourcesForArtifact(
  ...
  for (const example of db.prepare('SELECT input_artifact_ids_json, output_artifact_ids_json FROM work_discovery_examples').all()) {
    for (const field of ['input_artifact_ids_json', 'output_artifact_ids_json']) {
      try {
        const ids: unknown = JSON.parse(String(example[field]));
        if (!Array.isArray(ids) || ids.some(id => typeof id !== 'string') || ids.includes(artifactId)) count++;
      } catch { count++; }
    }
  }
  return count;
