...
export class WorkspaceSourceService {
  ...
  constructor(
    ...
    this.recovery = Promise.resolve().then(() => this.resumePendingSources()).catch(() => undefined);
  }
  ...
  stopAccepting(): void { this.ingestQueue.stopAccepting(); }
  ...
  async attachFile(sessionId: string, filePath: string, mimeType?: string): Promise<WorkspaceSourceRecord> {
    ...
    this.ingestQueue.assertCapacity();
