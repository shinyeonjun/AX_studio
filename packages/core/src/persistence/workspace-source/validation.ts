...
export function assertSessionId(sessionId: string): string {
  ...
  if (value === 'report-checkpoints') throw new WorkspaceSourceError('invalid_workspace_session');
...
export function errorMessage(code: string): string {
  if (code === 'workspace_source_queue_full') return '대기 중인 자료가 많습니다. 분석 완료 후 다시 올려 주세요.';
