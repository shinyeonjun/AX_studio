 ...
 * a new report. The chat host is the only layer that sees the current utterance,
 * so it must make the fresh-vs-resume decision before queueing side effects.
 * Direct command callers omit userMessage and retain the explicit API behavior.
 */
