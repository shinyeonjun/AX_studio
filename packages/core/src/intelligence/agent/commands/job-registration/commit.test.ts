...
describe('job commit and fail-closed execution', () => {
  it('drops an abandoned draft when its owning conversation is released', async () => {
    const { service, chat } = await connectedService();
    await service.execute({ name: 'job.propose', args: dailyBriefArgs }, {
      ...commandChatContext, workspaceSessionId: chat.id,
    });
    service.releaseWorkspaceSession(chat.id);
    const committed = await service.execute({ name: 'job.commit', args: {} }, {
      ...commandChatContext, workspaceSessionId: chat.id, allowJobCommit: true,
    });
    expect(committed.status).toBe('not_found');
  });
