...
describe('design-tools untrusted source policy', () => {
  it('blocks untrusted PDF body text when the caller has no local-data permission', async () => {
    ...
    setDocumentEngineClient({
      ...
      pdfReportAnalyze: async () => { throw new Error('unused'); }, ping: async () => true, ingest: async () => { throw new Error('must_not_ingest'); }, pdfToHtml: async () => { throw new Error('unused'); }, getChunk: async () => { throw new Error('unused'); }, getPage: async () => { throw new Error('unused'); }, search: async () => { throw new Error('unused'); } });
    try {
