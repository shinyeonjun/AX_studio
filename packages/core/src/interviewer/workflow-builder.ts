...
function insertApprovals(steps: Step[]): Step[] {
  const out: Step[] = [];
  for (const step of steps) {
    if (step.type === 'action' && step.sideEffect === 'EXTERNAL_HIGH') {
      const already = steps.some(
        (candidate) => candidate.type === 'human_approval' && candidate.forActionIds.includes(step.id),
      );
      if (!already) {
        out.push({
          type: 'human_approval',
          id: `approve_${step.id}`,
          reason: `${step.connector}.${step.action} 실행 전 승인`,
...
export function buildIRFromWorkflow(draft: InterviewDraft): Partial<SkillIR> {
  const steps = insertApprovals(draft.nodes.map(toStep).filter((step): step is Step => step !== null));
