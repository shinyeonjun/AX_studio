...
async function readClaudeSettingsModels(): Promise<CliModelOption[]> {
  const settingsPath = join(homedir(), '.claude', 'settings.json');
  try {
    const raw = JSON.parse(await readFile(settingsPath, 'utf8')) as { availableModels?: unknown; model?: unknown };
    const extras: CliModelOption[] = [];
    if (Array.isArray(raw.availableModels)) {
      for (const item of raw.availableModels) {
        if (typeof item === 'string') extras.push({ id: item, label: item });
      }
    }
    if (typeof raw.model === 'string') extras.unshift({ id: raw.model, label: raw.model });
    return extras;
  } catch {
    return [];
  }
...
export async function detectAiCliProviders(): Promise<DetectedAiCli[]> {
  ...
  for (const id of ids) {
    ...
    const detectedModels =
      id === 'claude-cli'
        ? normalizeModelOptions(await readClaudeSettingsModels())
        : id === 'codex-cli'
