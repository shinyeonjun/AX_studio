...
export const DOCUMENT_CAPABILITIES: ConnectorCapability[] = [
  ...
  {
    id: 'document.pdf.form.analyze',
    connector: 'document',
    kind: 'write',
    label: 'PDF 양식 분석',
    description: '원본 PDF를 기준으로 입력 필드 템플릿 생성',
    sideEffect: 'REVERSIBLE',
    params: [
      { name: 'path', label: 'PDF 경로', question: '어떤 PDF 양식을 분석할까요?', required: true },
      { name: 'ocr', label: 'OCR', question: '스캔 양식에 OCR을 사용할까요?', required: false },
      { name: 'fieldHints', label: '필드 힌트', question: '검토된 입력 필드 위치가 있나요?', required: false },
    ],
  },
  {
    id: 'document.pdf.form.fill',
    connector: 'document',
    kind: 'write',
    label: 'PDF 양식 작성',
    description: '원본 PDF 위에 검토된 값을 채워 PDF로 저장',
    sideEffect: 'REVERSIBLE',
    params: [
