...
|------|--------|-----------|--------------|
| **Read** | `packages/document-engine/` (Python) + `documents/read/` (TS client) | `read/` | `document.ingest`, `document.getChunk`, `document.getPage`, `document.search` |
| **Write** | `documents/write/` (Node) + `packages/document-engine/` (Python form path) | `write/` | `document.html.render`, `document.pdf.generate`, `document.docx.fill`, `document.pdf.toHtml`, `document.pdf.form.analyze`, `document.pdf.form.fill` |
...
trigger (local_folder / manual)
  → connectors/document/read/  (resolveDocumentIngestExecution)
  → documents/read/ TS client  →  Python worker (Docling | Basic)
...
```text
document.pdf.toHtml
  → documents/write/pdf/to-html  →  Python worker pdf_to_html (Docling export_to_html)
...
ai_decision (report body)
  → connectors/document/write/html  →  documents/write/html/render
  → connectors/document/write/pdf   →  documents/write/pdf/generate  →  DesktopPrintBridge
...
1. Implement pure logic in `documents/write/<format>/`.
2. Add connector adapter in `connectors/document/write/<format>/`.
3. Register the connector in `connectors/document/write/registry.ts`.

Path-based local file I/O: `connectors/local-folder/`.
