...
export async function pollSlackNewMessages(
  ...
  const cachedChannelMatches = params.cursorChannel === params.channel
    || (!params.initialized && !params.cursorChannel);
  ...
  if (!channelId) {
    channelId = await resolveSlackChannelId(client, params.channel, signal);
