...
describe('SlackConnector', () => {
  it('does not start a request when the host has cancelled', async () => {
    const controller = new AbortController();
    controller.abort();
    const result = await new SlackConnector('test-token').execute('channels.list', {}, {
      executionId: 'run-1', variables: {}, log: () => {}, abortSignal: controller.signal,
    });
    expect(result).toEqual({ ok: false, error: 'cancelled', errorCode: 'cancelled' });
  });
