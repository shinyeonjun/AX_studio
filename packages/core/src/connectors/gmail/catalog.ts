...
export const GMAIL_CAPABILITIES: ConnectorCapability[] = [
  ...
  },
  {
    ...
    kind: 'read',
    label: '메일 검색',
    description: '조건으로 메일 한 페이지 검색(기본 10, 최대 50건). truncated=true이면 같은 query/limit과 nextPageToken을 pageToken으로 전달해 계속 조회합니다. 선택적 nextPageToken/total/resultSizeEstimate/totalIsEstimate는 반환 metadata이며 필수 IO 포트가 아닙니다. total/resultSizeEstimate는 Gmail의 추정치입니다. completeness.status는 전체 검색 결과 기준이며 이어 읽은 마지막 페이지도 partial입니다.',
    sideEffect: 'NONE',
    params: [
