...
function extractEqLiteral(condition: ConditionExpr | undefined): string | undefined {
  if (!condition || condition.op !== 'eq' || !('lit' in condition.right)) return undefined;
...
function roleHintForLiteral(literal: string | undefined): string | undefined {
  return literal?.trim() ? nodeRoleHint(`branch_${literal.trim()}`) : undefined;
}
function actionIdsInBranch(stepIds: string[], byId: Map<string, Step>): string[] {
  return stepIds.filter((id) => byId.get(id)?.type === 'action');
}
function collectEnumLiterals(ir: Partial<WorkflowIR>): string[] {
  for (const step of ir.steps ?? []) {
    ...
    for (const definition of Object.values(properties as Record<string, unknown>)) {
      ...
      if (Array.isArray(enumValues) && enumValues.length >= 2) return enumValues.map(String);
    }
  }
  return [];
}
export function branchHintsFromWorkflow(ir: Partial<WorkflowIR>): Map<string, string> {
  ...
  function assign(stepIds: string[], literal: string | undefined) {
    ...
    if (!hint) return;
    for (const id of actionIdsInBranch(stepIds, byId)) if (!hints.has(id)) hints.set(id, hint);
  }
  function walkIf(ifId: string): void {
    ...
    if (elseIds.length === 1 && byId.get(elseIds[0]!)?.type === 'if') { walkIf(elseIds[0]!); return; }
    const remaining = collectEnumLiterals(ir).filter((value) => !usedLiterals.includes(value.toLowerCase()));
    assign(elseIds, remaining[0] ?? 'normal');
  }
  for (const step of steps) if (step.type === 'if') walkIf(step.id);
