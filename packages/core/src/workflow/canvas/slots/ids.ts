...
const NODE_ROLE_HINTS: Record<string, string> = {
  ...
  normal: '보고(normal)', report: '보고', 긴급: '긴급', 운영: '운영', 보고: '보고',
};

export function nodeParamSlotId(nodeId: string, paramName: string): string { return `${nodeId}.params.${paramName}`; }
export function nodeMemoSlotId(nodeId: string): string { return `${nodeId}.memo`; }
...
export function nodeSlotQuestion(nodeId: string, question: string, branchHint?: string): string {
  ...
  if (/Slack 채널|슬랙 채널/i.test(question)) return `${hint} 알림을 보낼 Slack 채널은 어디인가요?`;
  if (/Gmail|메일|수신자|받는/i.test(question)) return `${hint} 알림을 보낼 메일 주소를 알려주세요.`;
  if (/AI 단계|판단|분류할까요|무슨 내용|메시지/i.test(question)) return question;
