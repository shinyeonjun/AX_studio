...
function reportPeriodMetadata(period: ReportPeriod): Record<string, ReportPrimitive> {
  ...
  return {
    ...
    periodRange: `${period.start} ~ ${period.endInclusive}`,
    periodYear: start.year,
    periodMonth: start.month,
    periodMonthPadded: String(start.month).padStart(2, '0'),
    periodYearMonth: `${start.year}-${String(start.month).padStart(2, '0')}`,
...
export function reportExecutionMetadata(
  ...
  metadata.httpSourceLabel = capturePlan.http.length
    ? `REST  GET ${capturePlan.http.map((source) => source.path).join(' / ')}`
    : '';
  metadata.rdbSourceLabel = capturePlan.rdb.length
    ? `PostgreSQL  ${capturePlan.rdb.map((source) => unqualifiedTableName(source.table).replace(/_/g, ' ')).join(' / ')}`
    : '';
  ...
  for (const source of capturePlan.http) {
    ...
    // Some provider wire schemas group source metadata by connector kind.
    // Keep that host-owned alias alongside the flat key so a model cannot turn
    // a harmless naming variation into an execution failure.
    metadata[`source.http.${source.alias}.path`] = source.path;
  }
