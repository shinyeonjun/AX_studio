...
import { isReportAggregateExpression } from './schema.js';
import type {
...
function scalarDependsOnRuntime(expression: ReportScalarExpression): boolean {
  if (isReportAggregateExpression(expression)) return aggregateDependsOnRuntime(expression);
  switch (expression.kind) {
    case 'field': return true;
    case 'literal': return false;
    case 'arithmetic':
      return scalarDependsOnRuntime(expression.left) || scalarDependsOnRuntime(expression.right);
    case 'coalesce':
    case 'concat':
      return expression.values.some(scalarDependsOnRuntime);
  }
}

function isStaticPresentationScalar(expression: ReportScalarExpression): boolean {
  if (isReportAggregateExpression(expression)) return false;
  const isSafeText = (value: unknown): value is string => (
    typeof value === 'string' && !/\d/u.test(value)
  );
  const visit = (value: ReportScalarExpression): boolean => {
    if (isReportAggregateExpression(value)) return false;
    switch (value.kind) {
      case 'literal': return isSafeText(value.value);
      case 'concat':
      case 'coalesce': return value.values.every(visit);
...
function derivedExpressionDependsOnRuntime(expression: ReportDerivedExpression): boolean {
  if (isReportAggregateExpression(expression)) return aggregateDependsOnRuntime(expression);
  switch (expression.kind) {
    ...
    case 'literal': return false;
    case 'arithmetic':
      return derivedExpressionDependsOnRuntime(expression.left)
        || derivedExpressionDependsOnRuntime(expression.right);
    case 'coalesce':
    case 'concat':
      return expression.values.some(derivedExpressionDependsOnRuntime);
    case 'case':
      return expression.branches.some((branch) => (
        derivedPredicateDependsOnRuntime(branch.when)
        || derivedExpressionDependsOnRuntime(branch.value)
      )) || derivedExpressionDependsOnRuntime(expression.fallback);
  }
}

function derivedPredicateDependsOnRuntime(predicate: ReportDerivedPredicate | ReportOutputPredicate): boolean {
  switch (predicate.kind) {
    case 'compare':
      return derivedExpressionDependsOnRuntime(predicate.left)
        || derivedExpressionDependsOnRuntime(predicate.right);
    case 'in':
      return derivedExpressionDependsOnRuntime(predicate.value)
        || predicate.values.some(derivedExpressionDependsOnRuntime);
    case 'and':
    case 'or':
      return predicate.items.some(derivedPredicateDependsOnRuntime);
    case 'not':
      return derivedPredicateDependsOnRuntime(predicate.item);
    case 'is_null':
      return derivedExpressionDependsOnRuntime(predicate.value);
  }
}
...
export function assertReusableReportPlan(
  ...
  for (const scalar of plan.scalars) {
    if (!scalarDependsOnRuntime(scalar.expression)
      && !isStaticPresentationScalar(scalar.expression)) {
  ...
  for (const table of plan.tables) {
    ...
    for (const column of table.columns) {
      ...
      if (column.value.kind === 'derived' && !derivedExpressionDependsOnRuntime(column.value.expression)) {
        throw new Error(`report_plan_output_not_source_derived:table.${table.id}.column.${column.id}`);
      }
    }
  }
...
export function assertReusableReportPresentation(
  ...
  for (const text of plan.texts) {
    ...
    if (!slot || normalizeReportText(slot.exampleText) !== normalizeReportText(expected)) {
