...
function valueAtPath(value: unknown, path: string): unknown {
  ...
  let current = value;
  for (const segment of (path.startsWith('$.') ? path.slice(2) : path).split('.')) {
...
async function captureHttpSource(
  ...
  for (let pageIndex = 0; pageIndex < totalPages; pageIndex += 1) {
    ...
    if (spec.pagination && pageIndex >= spec.pagination.maxPages) {
    ...
    if (!isCompleteHttpPage(response.data) ||
      (!spec.pagination && response.data.completeness.status !== 'complete')) {
    ...
    if (spec.pagination) {
      ...
      if (pageIndex > 0 && reported !== totalPages) {
        throw new Error(`report_http_total_pages_changed:${spec.alias}:${page}`);
      }
      if (reported === 0 && pageRows.length > 0) {
        throw new Error(`report_http_total_pages_inconsistent:${spec.alias}:${page}`);
      }
      if (spec.pagination.currentPagePath && valueAtPath(json, spec.pagination.currentPagePath) !== page) {
        throw new Error(`report_http_page_mismatch:${spec.alias}:${page}`);
      }
      totalPages = reported;
      if (pageIndex + 1 >= totalPages && response.data.completeness.hasMore === true) {
        throw new Error(`report_http_total_pages_inconsistent:${spec.alias}:${page}`);
      }
    }
    consume(pageRows);
